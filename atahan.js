...
client.on("interactionCreate", async(int) => {
  if(int.isChatInputCommand()) {
    ...
    let reply = function(veri) {
      int.reply({embeds:[new EmbedBuilder().setTitle(`Uptime VDS Bot`).setAuthor({name:int.user.tag, iconURL:int.user.displayAvatarURL({dynamic:true})}).setDescription(veri).setFooter({text:`Made By Shiba`})], ephemeral:true})
    }
